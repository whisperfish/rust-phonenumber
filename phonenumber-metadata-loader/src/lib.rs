...
pub fn load<R: BufRead>(reader: R) -> Result<Vec<Metadata>, error::MetadataLoadError> {
    metadata(&mut Reader::from_reader(reader))
}

fn metadata<R: BufRead>(reader: &mut Reader<R>) -> Result<Vec<Metadata>, error::MetadataLoadError> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::End(ref e) if e.name().into_inner() != b"phoneNumberMetadata" => {
                return Err(error::MetadataParseError::MismatchedTag(
            ...
            event => {
                return Err(error::MetadataParseError::UnhandledEvent {
                    phase: "metadata".into(),
                    event: format!("{:?}", event),
                }
                .into())
...
fn territories<R: BufRead>(
    reader: &mut Reader<R>,
) -> Result<Vec<Metadata>, error::MetadataLoadError> {
    let mut buffer = Vec::new();
    let mut result = Vec::new();
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Eof => return Err(error::MetadataParseError::UnexpectedEof.into()),
...
fn territory<R: BufRead>(
...
) -> Result<Metadata, error::MetadataLoadError> {
    let mut buffer = Vec::new();
    ...
    for attr in e.attributes() {
        ...
        match (str::from_utf8(key.into_inner())?, str::from_utf8(&value)?) {
            ...
            (name, value) => {
                return Err(error::MetadataParseError::UnhandledAttribute {
                    phase: "format".into(),
                    name: name.into(),
                    value: value.into(),
                }
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Start(ref e) => match e.name().into_inner() {
                ...
                name => {
                    return Err(error::MetadataParseError::UnhandledElement {
...
fn descriptor<R: BufRead>(
    ...
    meta: &Metadata,
    name: &[u8],
) -> Result<Descriptor, error::MetadataLoadError> {
    let mut buffer = Vec::new();
    let mut descriptor = meta.defaults.descriptor.clone();

    fn lengths(value: &str) -> Result<Vec<u16>, error::MetadataLoadError> {
        let mut result = Vec::new();
...
fn formats<R: BufRead>(
    ...
    meta: &Metadata,
    name: &[u8],
) -> Result<(Vec<Format>, Vec<Format>), error::MetadataLoadError> {
    let mut buffer = Vec::new();
...
fn format<R: BufRead>(
    ...
    name: &[u8],
    e: &events::BytesStart<'_>,
) -> Result<(Format, Option<Format>), error::MetadataLoadError> {
    let mut buffer = Vec::new();
...
fn ignore<R: BufRead>(reader: &mut Reader<R>, name: &[u8]) -> Result<(), error::MetadataLoadError> {
    let mut buffer = Vec::new();
...
fn text<R: BufRead>(
    reader: &mut Reader<R>,
    name: &[u8],
) -> Result<String, error::MetadataLoadError> {
    let mut buffer = Vec::new();
